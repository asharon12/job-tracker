...
export default function StatCards({ stats }: { stats: Stats }) {
  const cards = [
    { label: 'Total Applications', value: stats.total, color: 'text-[var(--text)]' },
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {cards.map(({ label, value, color }) => (
        <div key={label} className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl p-5">
          <div className="text-xs text-[var(--text-muted)] font-medium uppercase tracking-wider mb-2">{label}</div>
